...
type Action =
  ...
  | { type: 'SET_MEDIA_PROVIDER'; provider: MediaProvider }
  | { type: 'QUEUE_VISUAL_DIRECTION_GENERATION' }
  | { type: 'START_VISUAL_DIRECTION_GENERATION'; prompt: string; kind: VisualMediaKind }
  | { type: 'SET_VISUAL_DIRECTION_JOB'; jobId: string }
  | { type: 'COMPLETE_VISUAL_DIRECTION_GENERATION'; assetUrl: string; kind: VisualMediaKind }
  | { type: 'FAIL_VISUAL_DIRECTION_GENERATION'; error: string }
  ...
  switch (action.type) {
    ...
    case 'GENERATE_SUGGESTIONS': {
      ...
      return {
        ...
        project: {
          ...
          directions,
          visualDirectionMedia: {
            ...state.project.visualDirectionMedia,
            status: 'queued',
            prompt: null,
            assetUrl: null,
            jobId: null,
    ...
    case 'SET_MEDIA_PROVIDER': {
      ...
      return {
        ...
        project: {
          ...state.project,
          visualDirectionMedia: { ...state.project.visualDirectionMedia, provider: action.provider },
        },
      }
    }
    case 'QUEUE_VISUAL_DIRECTION_GENERATION': {
      if (!state.project) return state
      return {
        ...state,
        project: {
    ...
    case 'START_VISUAL_DIRECTION_GENERATION': {
      if (!state.project) return state
      return {
        ...state,
        project: {
    ...
    case 'SET_VISUAL_DIRECTION_JOB': {
      if (!state.project) return state
      return {
        ...state,
        project: {
          ...state.project,
          visualDirectionMedia: { ...state.project.visualDirectionMedia, jobId: action.jobId },
        },
      }
    }
    case 'COMPLETE_VISUAL_DIRECTION_GENERATION': {
      if (!state.project) return state
      return {
        ...state,
        project: {
    ...
    case 'FAIL_VISUAL_DIRECTION_GENERATION': {
      if (!state.project) return state
      return {
        ...state,
        project: {
